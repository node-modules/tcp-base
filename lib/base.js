...
class TCPBase extends Base {
  ...
  send(packet, callback = noop) {
    ...
    this._invokes.set(packet.id, {
      ...
      timer: setTimeout(() => {
        ...
        err.message = `Server no response in ${timeout}ms, address#${this[addressKey]}`;
        err.socketMeta = meta;
  ...
  _connect(done) {
    ...
    socket.setTimeout(this.options.connectTimeout, () => {
      err.message = `[TCPBase] socket connect timeout (${this.options.connectTimeout}ms)`;
  ...
  _startHeartbeat() {
    ...
    this._heartbeatTimer = setInterval(() => {
      ...
      if (this._lastReceiveDataTime && duration > this.options.heartbeatInterval) {
        err.message = `server ${this[addressKey]} no response in ${duration}ms, maybe the socket is end on the other side.`;
